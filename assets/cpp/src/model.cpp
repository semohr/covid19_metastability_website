...
void Model::set_initials(
	...
	SV initials = {_S,_E_Q,_E_H,_I_Q,_I_H,_I_Hs,_R};
...
void Model::set_Phi(TimeDependentParameter p){
	Phi = p;
};
TimeDependentParameter Model::get_k(){
	return k;
};
TimeDependentParameter Model::get_Phi(){
	return Phi;
};
...
void data_struct::calc_Ns(Model model){
		N = get_new_cases(*this,model);
		N_obs = get_new_cases_obs(*this,model);
}