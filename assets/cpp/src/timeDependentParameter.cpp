...
void TimeDependentParameter::update_change(int index, double center, double length, double next_value){
	/*
	Updates the values for a changepoint via its index i.e. 0,1,2...
	*/
	change_points[index].center = center;
	change_points[index].length = length;
	change_points[index].next_value = next_value;
}
...
void TimeDependentParameter::update_inputevent(int index, double mean, double variance, double change){
	input_events[index].mean = mean;
	input_events[index].variance = variance;
	input_events[index].change = change;
}
